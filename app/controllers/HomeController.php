...
class HomeController extends BaseController {
 ...
 //        if(count($orders) > 0){
	...
	// 		foreach($orders as $o){
	// 			if($first){
	// 				$cur_stat = $o->status_id;
	//         		$first = false;
	//         		continue;
	//         	}

	//         	//echo $o->status_id . " " . $cur_stat . "<br/>";
	//         	if($o->status_id == $cur_stat){
	//         		$count++;
	//         	} else {
 ...
 //        //echo "Count: " . count($statuses);
	...
	// 	return View::make('admin.dashboard')->with('statuses', $statuses)->with('orderCount', $countOrders);	
	// }
